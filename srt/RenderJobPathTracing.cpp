...
void RenderJobPathTracing::Execute( )
{
	...
	const uint32_t surfPitchBB = m_context.backBuffer->GetMipDesc( 0 ).pitch;
	uint8_t * surfBB = reinterpret_cast< uint8_t * >( m_context.backBuffer->LockMipSurface( 0 ) );
	
	const uint32_t surfPitchPrev = m_context.prevResult->GetMipDesc( 0 ).pitch;
	...
	{
		uint32_t * lineBB = reinterpret_cast< uint32_t * >( surfBB + ( m_context.y + y ) * surfPitchBB ) + m_context.x;
		Vec3 * linePrev = reinterpret_cast< Vec3 * >( surfPrev + ( m_context.y + y ) * surfPitchPrev ) + m_context.x;
		Vec3 * lineCur = reinterpret_cast< Vec3 * >( surfCur + ( m_context.y + y ) * surfPitchCur ) + m_context.x;
